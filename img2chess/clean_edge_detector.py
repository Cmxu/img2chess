...
class CleanEdgeBasedDetector:
    ...
    def detect_board(self, image: np.ndarray) -> Optional[np.ndarray]:
        ...
        # Step 3: Filter lines by angle
        vertical_lines, horizontal_lines = self._filter_lines_by_angle(lines)
        
        # Step 4: Compute all distances
        all_distances = self._compute_all_distances(vertical_lines, horizontal_lines)
        if not all_distances:
            return None
        
        # Step 5: Find frequent distances
        frequent_distances = self._find_frequent_distances(all_distances)
        if not frequent_distances:
            return None
        
        # Step 6: Find valid square size
        square_side_length = self._find_valid_square_size(all_distances, frequent_distances)
        if square_side_length is None:
            return None
        
        # Step 7: Select lines matching chess pattern
        selected_vertical = self._select_lines_matching_pattern(
            vertical_lines, 'vertical', square_side_length)
        selected_horizontal = self._select_lines_matching_pattern(
            horizontal_lines, 'horizontal', square_side_length)
        
        # Step 8: Iterative boundary filtering
        selected_horizontal, selected_vertical = self._iterative_boundary_filtering(
            selected_horizontal, selected_vertical)
        
        # Step 9: Smart duplicate removal
        if len(selected_vertical) > 0:
            selected_vertical = self._remove_smart_duplicates(
                selected_vertical, 'vertical', square_side_length)
        if len(selected_horizontal) > 0:
            selected_horizontal = self._remove_smart_duplicates(
                selected_horizontal, 'horizontal', square_side_length)
        
        # Step 10: Validate we have enough lines
        min_lines = self.config['board_validation']['min_lines_required']
        if len(selected_vertical) < min_lines or len(selected_horizontal) < min_lines:
            return None
        
        # Step 11: Calculate board corners
        corners = self._calculate_board_corners(selected_vertical, selected_horizontal)
        if corners is None:
            return None
        
        # Step 12: Validate corner geometry
        if not self._validate_corner_geometry(corners, square_side_length):
            return None
        
        # Step 13: Extract and return board
    ...
    def _compute_all_distances(self, vertical_lines: np.ndarray, horizontal_lines: np.ndarray) -> List[float]:
        ...
        if len(vertical_lines) >= 2:
            ...
            all_distances.extend(self._compute_distances_for_direction(v_coords))
        
        # Horizontal distances
        if len(horizontal_lines) >= 2:
            h_coords = [(line[1] + line[3]) / 2 for line in horizontal_lines]
            all_distances.extend(self._compute_distances_for_direction(h_coords))
        
        return all_distances
    
    def _compute_distances_for_direction(self, coords: List[float]) -> List[float]:
        """Compute all pairwise distances for coordinates in one direction."""
    ...
    def _iterative_boundary_filtering(self, horizontal_lines: np.ndarray, vertical_lines: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        """Iteratively filter lines by boundaries of other direction."""
        ...
        for iteration in range(10):  # Max 10 iterations
            prev_h_count = len(horizontal_lines)
            prev_v_count = len(vertical_lines)
            
            horizontal_lines, vertical_lines = self._filter_lines_by_boundaries(
                horizontal_lines, vertical_lines, buffer_pixels)
            
            # Stop if no changes
            if len(horizontal_lines) == prev_h_count and len(vertical_lines) == prev_v_count:
                break
    ...
    def _remove_smart_duplicates(self, lines: np.ndarray, direction: str, square_side_length: float) -> np.ndarray:
